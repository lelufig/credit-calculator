...
def major(department, cource):
    if request.method == 'POST':
        ...
        sum_credit = dict()
        sum_credit['必要単位数'] = subjects_sum(my_credits.keys(), my_credits)
        ...
        sum_credit['♢印の中から2単位'] = subjects_sum([22010060, 22010070, 22140010, 22140020, 22140050, 22410010, 22410020, 22410030, 22410040, 22410050, 22410060, 22510340, 22510350, 22520090], my_credits)
        # excelで言うところのsheet3と4の単位が卒業要件単位にはまだ含まれてません。
        sum_credit['卒業要件単位（124単位以上）'] = subjects_sum(my_credits.keys(), my_credits)
        lack_credit = dict()
        lack_credit['必要単位数'] = max(0, 94 - sum_credit['必要単位数'])
        lack_credit['導入科目必修'] = max(0, 8 - sum_credit['導入科目必修'])
        ...
        lack_credit['♢印の中から2単位'] = max(0, 2 - sum_credit['♢印の中から2単位'])
